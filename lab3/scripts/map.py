...
def UpdateMap(xRob,yRob,theta, scan_msg):
    global updataCounter

    probObj = numpy.zeros((200,200))#initialize prob. observation
    counter = numpy.zeros((200,200))

    x = xRob*20
    y = yRob*20

    for laserIdx in xrange(640):#xrange(len(scan_msg.ranges)):
        dist = scan_msg.ranges[laserIdx]
        dist = dist*20
	...
	#print(xObj,yObj) 
        ...
        if(y>=200):y= 200-1
        xObj = int(round(xObj))
        if(xObj>=200):xObj= 200-1
        yObj = int(round(yObj))
        if(yObj>=200):yObj= 200-1
        ...
        for p in points:
            counter[p[0]][p[1]] += 1
        if(dist<(5*20)):probObj[xObj][yObj] += 1 #if detect object nearer than 5m
    ...
    for ix in xrange(probObj.shape[0]):
       for iy in xrange(probObj.shape[1]):
		...
		if(probObj[ix][iy] >= 0.5):#believe obstacles
...
class MAPPER:
    def __init__(self):
    	...
    	ts = message_filters.ApproximateTimeSynchronizer([pose_sub, scan_sub], 1, 1000)
    ...
    def callback(self, odom_msg, scan_msg):
        ...
        if(abs(x - xOld)>0.05 or abs(y - yOld)>0.05 or abs(theta - thetaOld)>0.5):
            UpdateMap(x,y,theta, scan_msg) #fun for updating grid map
            ...
            for ix in xrange(mapData.shape[0]):
                for iy in xrange(mapData.shape[1]):
                    ...
                    for m in xrange(2):
                        for n in xrange(2):
                            img.putpixel(((ix*2)+m,(200-1-iy)*2 + n), (clr,clr,clr))
